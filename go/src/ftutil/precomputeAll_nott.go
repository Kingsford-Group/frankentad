...
func PrecomputeAll_nott(hicmap map[hicutil.Pair]float64, chrlength int, iswindowsize int, p int, q int,res int) (map[hicutil.Pair]float64, [][]float64, []float64, []int, []float64, []float64, map[hicutil.Pair]float64, map[hicutil.Pair]float64, float64, []float64, []int) {
	...
	numdi := 0
        for b := 1; b < chrlength; b++ {
                ...
                for a := b-1; a > -1; a-- {
					...
					if a >= b-iswindowsize/2 {
						for c := b; c < b+iswindowsize/2+1; c++{
							fmt.Println(a,c, hicmap[hicutil.Pair{a,c}])
							//if b == 20 {fmt.Println(a,c,hicmap[hicutil.Pair{a,c}])}
						} // maybe missing the row where a = b because a starts at b-1
						fmt.Println(b,a+iswindowsize/2+1,hicmap[hicutil.Pair{b,a+iswindowsize/2+1}])
						//if b == 20 {fmt.Println(b,a+iswindowsize/2)}
					}
                                        os.Exit(1)
                                }
	...
	disignchanges = disignchanges[:numdi+1]
        ...
        for i := 0; i < chrlength; i++ {
				...
				//fmt.Println(i,allisscores[i])
					...
					fmt.Println(normfactor,sumisscores,chrlength-2*iswindowsize)
                                        os.Exit(1)
                                }
